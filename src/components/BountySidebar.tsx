...
export default function BountySidebar({ bounty, onClose, onAnswerSubmit, onAnswerAdd }: BountySidebarProps) {
  ...
  const toggleAnswer = async (answerId: string, currentlyAccepted: boolean) => {
    setOptimisticAnswers(prev => ({ ...prev, [answerId]: !currentlyAccepted }));
  ...
  return (
    <div className="bg-white h-full rounded-t-xl flex flex-col overflow-hidden pointer-events-auto" onClick={(e) => e.stopPropagation()}>
      <header className="px-4 py-3 border-b flex items-center sticky top-0 bg-white z-10">
        <h2 className="text-xl font-semibold text-gray-800 flex-1">Bounty Details</h2>
        <button onClick={onClose} className="p-2 -mr-2 active:bg-gray-100 rounded-full">
          ...
          <div className="flex justify-between items-start">
            ...
            {remainingTime && (
              <span className={`text-sm font-medium px-2 py-1 rounded-full ${
                remainingTime === 'Expired' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
              }`}>
                {remainingTime}
              </span>
            )}
              ...
              return (
                <div
                  ...
                  className={`p-3 border rounded-lg transition-all duration-200 cursor-pointer ${
                    isAccepted ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200 hover:bg-gray-50'
                  }`}
                >
                  <p className="text-gray-800">{answer.content}</p>
                  {answer.imageUrl && (
                    <div className="mt-2">
                      <Image src={answer.imageUrl} alt="Answer image" width={300} height={200} className="rounded-lg object-cover" />
                    </div>
                  )}
                  {isAccepted && <div className="mt-2 text-sm font-medium text-green-600">Accepted ✓</div>}
                </div>
              );
            })
          )}
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t bg-gray-50 sticky bottom-0">
        ...
        <div className="mt-2 flex gap-2">
          <button
            ...
            className="flex-1 bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-center font-medium"
          >
